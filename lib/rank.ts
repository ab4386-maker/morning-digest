...
3. A "cadence" — drives how long the item stays in the digest. **Default to "weekly" — be conservative on "today".**
   - "today" = TRULY ephemeral / time-sensitive news where the item stops being useful within ~2 days. ONLY: CPI/jobs prints, FOMC days, single-stock moves >5% on the day, breaking M&A on announcement day, breaking geopolitical shocks (strikes / sanctions / market-moving headlines). Hard cutoff: **items marked "today" are DROPPED from the digest after 48 hours.** Don't mark something "today" if you'd still want it visible 3 days from now.
   - "weekly" = anything with read-through that's still interesting in a week. Structural events with downstream implications (IPO debuts, REIT launches, fund raises, deal closes, sector setups), trend pieces, post-event analysis, regulatory shifts that take time to play out, anything analytical. These stay up to 60 days, decay naturally in ranking.
   When in doubt → "weekly". A Blackstone data-center REIT debut is "weekly" (the launch itself is one-time but the read-through to data-center demand, REIT sector flows, and Blackstone's capital allocation matters for weeks). A "Fed holds rates steady at 4.5%" headline on FOMC day is "today" (the news IS the day).
